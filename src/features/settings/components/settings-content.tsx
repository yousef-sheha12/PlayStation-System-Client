...
export default function SettingsContent() {
  ...
  const handleFormSubmit = (data: DeviceFormData) => {
    if (editingDevice) {
      updateDevice(
        {
          id: editingDevice.id,
          data: { ...data, status: editingDevice.status },
        },
        {
          onSuccess: () => {
            setFormOpen(false);
    ...
    updateDevice(
      {
        ...
        data: {
          name: rateModal.name,
          hourlyRate: newRate,
          status: String(statusMap[rateModal.status] ?? 0),
        },
      },
      {
  ...
  const handleProductSubmit = (data: ProductFormData) => {
    if (editingProduct) {
      updateProduct(
        {
          ...
          data: { ...data, categoryId: editingProduct.categoryId || 0 },
        },
        {
          onSuccess: () => {
      ...
      createProduct(
        { ...data, categoryId: 0, lowStockThreshold: 5 },
        {
          onSuccess: () => {
            setProductFormOpen(false);
            resetProduct();
  ...
  return (
    <div>
      ...
      <Card hover={false} className="mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-800">
                {t("settings.devicesManagement")}
              </h2>
              <p className="text-sm text-gray-500">
                {t("settings.devicesCount", { count: devices.length })}
              </p>
            </div>
          </div>
          {canManageDevices && (
        ...
        {devicesLoading ? (
          <div className="p-8 text-center">
            <Loader size="lg" text={t("settings.loadingSettings")} />
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="font-bold text-blue-600">
                    {formatCurrency(device.hourlyRate)}
                  </span>
                </div>
      ...
      <Card hover={false}>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-800">
                {t("settings.productsTitle")}
              </h2>
              <p className="text-sm text-gray-500">
                {t("settings.productsCount", { count: products.length })}
              </p>
            </div>
          </div>
          {/* <Button
      ...
      <Modal
        ...
        title={
          editingDevice ? t("settings.editDevice") : t("settings.addDevice")
        }
        size="sm"
      >
      ...
      <Modal
        ...
        title={t("settings.updateRate")}
        size="sm"
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-500">
            {t("settings.updateRateDesc", { device: rateModal?.name || "" })}
          </p>
          <Input
            label={t("settings.newHourlyRate")}
            type="number"
            step="0.01"
            value={newRate}
      ...
      >
        <form
          onSubmit={handleSubmitProduct(handleProductSubmit)}
          className="space-y-4"
        >
          <Input
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deleteId)
            deleteDevice(deleteId, { onSuccess: () => setDeleteId(null) });
        }}
